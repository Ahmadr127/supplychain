...
@section('header')
    <div class="flex items-center gap-2">
        <a href="{{ route('ts-categories.index') }}" class="text-gray-500 hover:text-gray-700">
            <i class="fas fa-arrow-left"></i>
        </a>
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Edit Kategori Technical Support: {{ $tsCategory->name }}
        </h2>
    </div>
@endsection
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-visible shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200 rounded-lg">
                ...
                <form action="{{ route('ts-categories.update', $tsCategory) }}" method="POST">
                    @csrf
                    @method('PUT')
                    
                    <div class="mb-4">
                        <label for="name" class="block text-sm font-medium text-gray-700">Nama Kategori <span class="text-red-500">*</span></label>
                        <input type="text" name="name" id="name" value="{{ old('name', $tsCategory->name) }}" required
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" id="description" rows="3"
                                  class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('description', $tsCategory->description) }}</textarea>
                        @error('description') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>

                    <div x-data="{ approverType: '{{ old('ts_approver_type', $tsCategory->ts_approver_type) }}' }">
                        <div class="mb-4">
                        ...
                        <div x-show="approverType === 'user'" class="mb-4" style="display: none;">
                            ...
                            <div x-data="{
                                open: false,
                                search: '',
                                selectedId: '{{ old('ts_approver_id', $tsCategory->ts_approver_id) }}',
                        ...
                        <div x-show="approverType === 'role'" class="mb-4" style="display: none;">
                            ...
                            <div x-data="{
                                open: false,
                                search: '',
                                selectedId: '{{ old('ts_approver_role_id', $tsCategory->ts_approver_role_id) }}',
                    ...
                    <div class="mb-6">
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', $tsCategory->is_active) ? 'checked' : '' }}
