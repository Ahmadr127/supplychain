...
document.addEventListener('DOMContentLoaded', function() {
        ...
        if (pName && pId && pBox) {
            ...
            pBox.addEventListener('click', function(e){
                ...
                if (found && unitPref && totalPref) {
                    const uv = parseRupiah(found.unit_price);
                    const tv = parseRupiah(found.total_price);
                    if (uv) unitPref.value = formatRupiah(uv);
                    if (tv) { totalPref.value = formatRupiah(tv); totalPref.dataset.manual = '1'; }
                }
            });
    ...
    if (bmForm) {
        bmForm.addEventListener('submit', function(){
            ...
            inputs.forEach(inp => { if (inp.value) inp.value = String(parseRupiah(inp.value)); });
        });
    }
});
</script>
