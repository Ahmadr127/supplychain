    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-2xl font-bold text-gray-900">Satuan Barang</h2>
                <a href="{{ route('units.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
            ...
            <div class="bg-gray-50 p-4 rounded-lg mb-6">
                <form method="GET" action="{{ route('units.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-2">
                                            ...
                                            <form action="{{ route('units.destroy', $unit) }}" method="POST" 
                                                  class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus satuan ini?')">
                                                @csrf
                                                @method('DELETE')
            ...
            @else
                <div class="text-center py-12">
                    <i class="fas fa-weight text-gray-400 text-6xl mb-4"></i>
                    <h3 class="text-lg font-medium text-gray-900 mb-2">Tidak ada satuan barang</h3>
                    <p class="text-gray-500 mb-4">Mulai dengan membuat satuan barang pertama Anda.</p>
                    <a href="{{ route('units.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                        <i class="fas fa-plus mr-1"></i> Tambah Satuan
                    </a>
                </div>
